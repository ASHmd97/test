...
void main()
{
    ...
    {
        vec2 curBottomCoordinate    = texCoordVarying+vec2(0.0, float(i))*unit_uv;
        vec2 curTopCoordinate       = texCoordVarying+vec2(0.0, float(-i))*unit_uv;
    ...
    // vec3 col = mix(result1, result2, smoothstep(0.3, 0.7, progress)).rgb;
